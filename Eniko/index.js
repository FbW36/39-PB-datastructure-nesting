...
const doggo = {
    name: 'Stinky',
    breed: 'danish bloodpointer',
    favoriteFood: ["bikkies", "chimkon", "garbage"],
};

console.log("The doggos favorite food is: ", doggo.favoriteFood[1]);

function favoriteFoodFunction (array) {
	for (let i in array) {
        if (i == "favoriteFood") {
            console.log(array[i]);
        }
...
const recipes = {
    ...
    ingedientsLoop: function() {
       for (let i in this.ingedients) {
           console.log("Ingridient amounts: ", this.ingedients[i]);
       }
    },
};
recipes.ingedientsLoop();

recipes.ingedients.ginger =  "2 tss";
console.log(recipes);
recipes.ingedients.brownSugar = recipes.ingedients.sugar ;
delete recipes.ingedients.sugar;
console.log(recipes);